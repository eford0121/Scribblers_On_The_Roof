...
def cnnOld(x_train, x_val, x_test, y_train, y_val, y_test, classescount):
    ...
    filepath = "Normalization/weights02.{epoch:02d}.h5"
    ES = EarlyStopping(patience=5)
    check = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, 
    _best_only=False, mode='max')
...
def lenet(x_train, x_val, x_test, y_train, y_val, y_test, num_classes):
    ...
    filepath = "Normalization/weightslenet.{epoch:02d}.h5"
    ES = EarlyStopping(patience=5)
    check = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True, mode='max')
